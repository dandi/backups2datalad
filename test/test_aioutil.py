...
def split_with_linesep(chunks: list[str]) -> list[str]:
    """The previous implementation, kept here as a test oracle."""
...
async def test_line_receive_stream_matches_linesep() -> None:
    """
    Randomised parity against the `linesep` splitter this class used to be
    built on, over inputs full of the awkward cases (bare CR, CRLF straddling a
    chunk boundary, empty chunks, unterminated tails).
    """
...
async def test_line_receive_stream_long_line_is_linear() -> None:
    """
    A single very long line must not cost time quadratic in its length.

    `git annex whereis --json` for a key with many registered URLs emits one
    JSON line of tens of megabytes, and the previous buffer-and-rescan
    implementation took ~33s to read 32 MiB of it -- which is what made Zarr
    backups crawl.  Reading it should now be a fraction of a second; the bound
    here is loose enough not to be flaky on a loaded machine but far below the
    quadratic cost.
    """
    line = "x" * (32 * 1024 * 1024) + "\n"
